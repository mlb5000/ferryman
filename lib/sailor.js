...
class Sailor {
  ...
  async prepare() {
    const {
      settings: { COMPONENT_PATH: compPath, FLOW_ID: flowId, STEP_ID: stepId },
      apiClient,
      componentReader,
    } = this;
  ...
  reportError(err) {
    const headers = Object.assign(
      {},
      getAdditionalHeadersFromSettings(this.settings),
      {
  ...
  scheduleShutdown() {
    return co(
      function* doScheduleShutdown() {
        if (this.shutdownCallback) {
          log.debug(
            "scheduleShutdown – shutdown is already scheduled, do nothing"
          );
  ...
  readIncomingMessageHeaders(message) {
    ...
    // Get meta headers
    const metaHeaderNames = Object.keys(headers).filter((key) =>
      key.toLowerCase().startsWith(AMQP_HEADER_META_PREFIX)
    );

    const metaHeaders = _.pick(headers, metaHeaderNames);
    const metaHeadersLowerCased = _.mapKeys(metaHeaders, (value, key) =>
      key.toLowerCase()
    );
    ...
    const result = {
      ...
      threadId:
        headers.threadId || metaHeadersLowerCased["x-eio-meta-trace-id"],
  ...
  async processMessage(payload, message) {
    ...
    const { deliveryTag, routingKey } = message.fields;
    ...
    let module;
    try {
      module = await this.componentReader.loadTriggerOrAction(
        settings.FUNCTION
      );
    } catch (e) {
      log.error(e);
    ...
    return new Promise((resolve) => {
      ...
      const taskExec = new TaskExec({
        loggerOptions: _.pick(incomingMessageHeaders, [
          "threadId",
          "messageId",
          "parentMessageId",
        ]),
      ...
      async function onData(data) {
        ...
        console.log(" why do I not see this message?");
        logger.trace(
          {
            headers,
          },
        ...
        if (stepData.is_passthrough === true) {
          logger.trace("stepData.is_passthrough");
          if (settings.NO_SELF_PASSTRHOUGH) {
            ...
            logger.trace("not stepData.is_passthrough");
            data.passthrough = Object.assign({}, origPassthrough, {
              [self.settings.STEP_ID]: Object.assign(
                {},
                _.omit(data, "passthrough")
              ),
            });
          }
        }
        ...
        try {
          logger.trace(
            {
              data,
              headers,
              throttleData: self.throttles.data,
            },
            "sending message data"
          );
          return await self.amqpConnection.sendBackChannel(
            data,
            headers,
            self.throttles.data
          );
          ...
          logger.trace(
            ...
            "processMessage emit error (before onError)"
          );
          return onError(err);
        }
      }
      ...
      async function onError(err) {
        ...
        headers.end = new Date().getTime();
        return self.amqpConnection.sendError(
          err,
          headers,
          message,
          self.throttles.error
        );
      }
      ...
      async function onUpdateSnapshot(data) {
        ...
        if (_.isPlainObject(data)) {
          if (data.$set) {
            return log.warn(
              "ERROR: $set is not supported any more in `updateSnapshot` event"
            );
          }
